...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            if((self.normalization=="batchnorm") or (self.normalization=="layernorm") ):
                self.l_params['gamma%d' % (i + 1)] = np.ones(dims[i+1])
                self.l_params['beta%d' % (i + 1)] =np.zeros(dims[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(hidden_num):
            if(self.normalization == "layernorm"):
                scores,cache = affine_layernorm_relu_forward(scores, self.params['W%d' % (i + 1)],
                                                              self.params['b%d' % (i + 1)],
                                                              self.l_params['gamma%d' % (i + 1)],
                                                              self.l_params['beta%d' % (i + 1)],
                                                              self.bn_params[i]
                                                              )
                cache_history.append(cache)
            
            if(self.normalization=="batchnorm"): 
                scores,cache = affine_batchnorm_relu_forward(scores, self.params['W%d' % (i + 1)],
                                                              self.params['b%d' % (i + 1)],
                                                              self.l_params['gamma%d' % (i + 1)],
                                                              self.l_params['beta%d' % (i + 1)],
                                                              self.bn_params[i])
        ...
        while i >= 0:
            ...
            if(self.normalization=="batchnorm"):
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)] = affine_batchnorm_relu_backward(dout, cache_history.pop())
            elif(self.normalization=="layernorm"):
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)] = affine_layernorm_relu_backward(dout, cache_history.pop())
