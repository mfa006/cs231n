...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embs = self.embedding(captions)
        encodings = self.positional_encoding(embs)
        proj_vector = self.visual_projection( features )
        image_vector = torch.unsqueeze(proj_vector,1)
        tgt_mask = torch.tril(torch.ones(T, T))
        output = self.transformer(encodings,image_vector,tgt_mask)
