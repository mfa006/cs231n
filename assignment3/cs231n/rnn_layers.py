...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(x.dot(Wx) + prev_h.dot(Wh) + b)  #ht =Activation((Whx⋅xt )+(Whh⋅ht−1 )+bh)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x.shape[1]):
      next_h, cache_h = rnn_step_forward(x[:,t], h[t], Wx, Wh, b)
      h.append(next_h)
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    (N, T, H), (D, E) = dh.shape, cache[0][3].shape
    dx = np.empty((N, T, D))
    dh0 = np.zeros((N, H))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
