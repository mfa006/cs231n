...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[:, :, even_ind] = torch. tensor([[math.sin(i*pow(10000, -j/embed_dim))for j in even_ind]for i in index])
        pe[:, :, odd_ind] = torch. tensor([[math.cos(1*pow(10000, - (j-1)/embed_dim))for j in odd_ind]for i in index])
